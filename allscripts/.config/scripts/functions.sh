...
infra() {
  ...
  ssh mamutal91@65.21.193.29 "cd $HOME && rm -rf /mnt/roms/infra && git clone ssh://git@github.com/AOSPK/infra /mnt/roms/infra" &> /dev/null
}
...
dot() {
  if [[ ${1} ]]; then
    ...
    ssh mamutal91@65.21.193.29 "cd $HOME && rm -rf .dotfiles && git clone ssh://git@github.com/mamutal91/dotfiles .dotfiles && bash $HOME/.dotfiles/install.sh && source $HOME/.zshrc" &> /dev/null
...
bkp() {
  ...
  ssh mamutal91@65.21.193.29 "bash $HOME/.config/scripts/gitcron.sh"
