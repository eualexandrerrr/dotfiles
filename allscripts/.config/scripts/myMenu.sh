...
chosen="$(echo -e "$menu" | rofi -no-config -no-lazy-grab -dmenu -i -theme ~/.config/polybar/$(grep "launch" $HOME/.dotfiles/install.sh | awk '{print $3}' | tr -d '-')/scripts/rofi/launcher.rasi)"
case $chosen in
