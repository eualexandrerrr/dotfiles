...
## Etapa `claude`: o ~/.claude sobrevive ao format

`setup.sh claude` mescla `claude/settings.json` do repo no `~/.claude/settings.json`
(**merge com jq, nunca cp por cima** -- o Claude Code grava tema, modelo e plugins nesse
mesmo arquivo) e copia os autorais: `CLAUDE.md`, `statusline.js`, `mcp-doctor.js`,
`commands/`, `skills/` e `bin/`.

Nao entra no stow de proposito: o `~/.claude` e reescrito pelo proprio Claude Code o tempo
todo, e save atomico (write + rename) troca symlink por arquivo real na primeira gravacao.

Fora do repo ficam `projects/` (2,5 GB de transcript), `sessions/`, `plugins/` (o
`settings.json` ja declara os marketplaces, o proprio Claude Code rebaixa) e
`.credentials.json` (esse vai no `segredos.tar.age`). `remoteControlAtStartup: true` esta no
settings versionado, entao Remote Control sobe ligado em toda maquina nova.
