...
# is located in vendor/aosp/tools. Other ROMs will need to change this. The logic is
# as follows:
...
if [ -z ${1} ]; then
  echo -e "You need to specify a tag: Example:\n.merge_aosp.sh 11.0.0_r38"
  exit
fi

WORKING_DIR=$( cd $( dirname $( readlink -f "${BASH_SOURCE[0]}" ) ) && pwd )
WORKING_DIR="/mnt/roms/jobs/KrakenDev"
